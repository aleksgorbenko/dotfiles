...
Out of scope (do not attempt): interface/protocol definitions, method signatures, function-level API contracts between packages. That belongs to a separate, later "architecture" skill once real code exists — do not pull that thread here even if the user starts describing methods.
...
6. If the user starts describing package interfaces or method signatures during round 3, note that this is deferred to a future architecture-focused skill and keep the round scoped to responsibility + dependency direction + placement.
